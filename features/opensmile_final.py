...
def process_session(
    ...
    *,
    ...
    # debug
    print("First 30 feature columns:", list(seg_feats.columns[:30]))
    print("F0-like cols:", [c for c in seg_feats.columns if "F0" in c][:30])
    print("loudness-like cols:", [c for c in seg_feats.columns if "loudness" in c][:30])
    print("jitter-like cols:", [c for c in seg_feats.columns if "jitter" in c][:30])
    print("shimmer-like cols:", [c for c in seg_feats.columns if "shimmer" in c][:30])
    print("HNR-like cols:", [c for c in seg_feats.columns if "HNR" in c or "hnr" in c][:30])
    print("mfcc1-like cols:", [c for c in seg_feats.columns if "mfcc1" in c.lower()][:30])
    ...
    for p in segment_df["seg_wav"].tolist():
        try:
            a, v = predict_arousal_valence(p, emo_processor, emo_model, device=device)
...
if __name__ == "__main__":
    ...
    out_dir = f"{core_path}/actual_features"
    participant_id = "P01"
    session_id = "P01_S03"
